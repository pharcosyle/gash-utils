  ...
  (match command
    (('<awk-item> ('<awk-pattern> pattern) action)
     (receive (expr env) (awk-expression->boolean pattern env)
       (if expr (run-commands action env) env)))
    (('<awk-item> (or ('<awk-begin>) ('<awk-end>)) action) env)
    (('<awk-pattern> _)
     (let ((command* `(<awk-item> ,command (<awk-print>))))
       (run-commands command* env)))
    (('<awk-action> actions ...)
     (fold run-commands env actions))
    (('<awk-expr> expr)
    ...
    (('<awk-for-in> key array action)
     (let* ((save-key (get-var key env))
            ...
            (env (fold (lambda (value env)
                         (run-commands action (assign key value env)))
                       env
                       keys)))
         ...
         (receive (init-expr env) (awk-expression init env)
           (let loop ((env env))
             (receive (test env) (awk-expression->boolean test env)
               (if (not test) env
                   (let ((env (run-commands action env)))
    ...
    (('<awk-while> test action)
     (let loop ((env env))
       (receive (result env) (awk-expression->boolean test env)
         (if result
             (loop (run-commands action env))
             env))))
    (('<awk-do> action test)
     (let loop ((env (run-commands action env)))
       (receive (result env) (awk-expression->boolean test env)
         (if result
             (loop (run-commands action env))
             env))))
    ...
    (((? symbol?) . rest)
     (receive (expr env) (awk-expression command env)
       env))
    ((lst ...) (fold (cut run-commands <> <>) env lst))
...
(define* (%eval-awk items names #:optional
  ...
  (let* ((env (make-default-env out field-separator))
         (env (eval-special-items items '(<awk-begin>) env))
         (env (fold (cut run-awk-file items <> <>) env names)))
    (eval-special-items items '(<awk-end>) env)))
